(() => {
  ...
  function validateRunTrial(raw) {
    ...
    if (!Array.isArray(params.job_ids) || params.job_ids.length < 1 || params.job_ids.length > 10) {
      invalidParams("params.job_ids", "expected 1-10 job ids (a development trial runs one continuous chain; larger batches are owner-clicked)");
    }
  ...
  const METHOD_ENTRIES = [
    ...
    registryEntry({ name: "run.trial", context: "executor", read_only: false, approval: "none", deadline_ms: 30000, description: "Start one owner-gated development trial run of at most 10 runnable jobs (one continuous chain); refused unless the side-panel development-mode toggle is ON, no run is active, and 300 seconds have passed since the previous trial. Larger production runs stay owner-only.", params_schema: { job_ids: "job_id[1..10]", timeout_sec: "integer:15..90?", delay_sec: "integer:20..30?" }, params_validator: validateRunTrial })
  ];
