...
5. `legacy: true` = profile đó còn chạy bản extension CŨ trong RAM — nhờ Đức reload extension ở profile đó rồi đặt tên.
   **NHƯNG `legacy: false` KHÔNG có nghĩa "đang chạy code mới nhất"** — cờ đó chỉ nói profile có
   bắt tay theo giao thức nhiều-hồ-sơ hay không. Đo thật 07/09: hai profile **cùng** `legacy:false`,
   **cùng** `extension_version: 0.2.0`, mà một cái 19 method và một cái 23 — cái 19 còn ôm bản
   trước 06/09 trong RAM. **Muốn biết profile nào chạy code mới thì ĐẾM METHOD** của
   `capabilities`, đừng đọc cờ `legacy` và đừng đọc `extension_version` (nó lấy từ manifest, mà
   manifest không đổi khi code đổi). Chạy lượt live vào profile code cũ là nghiệm thu một bản vá
   không có ở đó. Bằng chứng: `evidence-multiprofile-nghiem-thu-20260907/`.
6. Đổi tên hồ sơ: gõ vào ô rồi bấm **Lưu tên** (hoặc click ra ngoài ô) — panel lưu và tự nối lại đúng socket của profile đó, tên hiện trên `bridge.sessions` NGAY, không đụng host, không đụng profile khác. Lệnh đang bay của đúng profile đó (nếu có) nhận `TRANSPORT_DISCONNECTED` retryable — giống hệt một cú service worker thức dậy.
...
| `REQUEST_TIMEOUT` cho 1–3 lệnh CHỈ ĐỌC đầu tiên, rồi mọi lệnh trả lời dưới 1 giây | **Cửa sổ đánh thức service worker.** Sau một khoảng nghỉ, mấy lượt gọi đầu tiêu trọn 10 giây deadline mỗi lượt để dựng lại kênh; xong thì kênh ấm và mọi method trả lời tức thì | Gọi lại, đừng đổi đích, đừng đổi method. **Và đừng kết luận method nào hỏng riêng.** Đo thật 07/09: `run.status` timeout **5 trong 6** lượt liên tiếp trong khi `ledger.read` trả lời 6/6 — cùng context, cùng deadline, cùng đường dispatch. Nhìn y như "`run.status` hỏng", mà `run.status` là camera canh lúc bấm dừng nên kết luận đó chặn cả `G-01`. **Nó sai.** Phép đo tách được hai nguyên nhân là **xen kẽ ba method và in giờ từng lượt**: lỗi đóng theo THỜI GIAN (ba lượt đầu timeout, mỗi method đúng một lượt, rồi 15 lượt sau xanh trong 4 giây), không theo METHOD. Gọi liên tiếp MỘT method thì hai nguyên nhân nhìn giống hệt nhau. Vòng poll 10 giây lúc chạy tự giữ kênh ấm, nên giữa một run thì `run.status` tin được. Bằng chứng: `evidence-multiprofile-nghiem-thu-20260907/` mục `[10]` |
| Panel đang MỞ hay đã ĐÓNG — phân biệt thế nào mà không gõ gì lên trang | `EXECUTOR_UNAVAILABLE` = panel đóng (không có executor nào nhận). `WORKBOOK_NOT_LOADED` = panel **đang mở** và trả lời được, chỉ là chưa chọn workbook | Dùng `ledger.read` làm phép thử panel: nó chỉ-đọc, không cần workbook, và hai mã lỗi trên tách bạch hai trạng thái. Muốn biết có tab hội thoại Gemini đang hoạt động thì `diagnostics.dom_probe` — trả `"Open a normal Gemini conversation in the active tab."` là chưa có, và **không có lệnh Bridge nào mở tab hộ** (cố ý, xem `AGENTS.md` mục 7): đó là việc Đức bấm |
