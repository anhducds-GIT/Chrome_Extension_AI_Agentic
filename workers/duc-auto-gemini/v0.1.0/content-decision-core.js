(() => {
  "use strict";
  function blockingFailure(snapshot = {}) { return snapshot.security || snapshot.securityBlocker || snapshot.quota || snapshot.quotaBlocker || null; }
  ...
  async function exposeFileInput(ports) {
    ...
    ports.click(trigger); input = await ports.waitInput(); if (input) return input;
    const menuItem = ports.findMenuItem(); if (!menuItem) throw new Error("UPLOAD_MENU_ITEM_MISSING");
    ports.click(menuItem); input = await ports.waitInput(); if (!input) throw new Error("FILE_INPUT_NOT_EXPOSED"); return input;
  }
