...
- **Check Plan** validates workbook, references, effective settings, output destinations, permission, retry budget, and result filename without submitting a ChatGPT prompt. Run validates again authoritatively.
...
- References have a compact gallery with remove controls. Tokens resolve exact filename first, then extensionless basename; matching is case-insensitive and duplicates fail validation. **There is no alias UI.** The resolver still carries an alias branch ahead of those two, but every path that fills `state.files` — the file picker and `references.add` over the Bridge — writes `alias: ""`, and every alias branch is guarded on a non-empty key, so that branch has never executed. `tests/reference-alias-dead-code-static.mjs` measures this and turns red the day someone wires an alias input; read `B-20` in `BACKLOG.md` before doing so. Do not document or rely on aliases until then.
...
- Each run writes one stable `<base>__audit.jsonl` alongside its result location. Events contain attempt phase/status, task/output type, reference tokens (filename, since no alias UI exists), output filename or text response count/hash, target URL when available, and a prompt hash/length rather than duplicating prompt or response text. A profile-folder continuation reads and appends that existing audit before replacing it only after persistence verification. Image collision policy remains limited to images.
- Result checkpoints are **bounded**. Once a newer one is written and verified, older ones are deleted, keeping the newest `checkpoint_retention` (default 2). Three jobs write ten checkpoints, so a 66-job pilot would otherwise leave roughly two hundred files behind. The newest checkpoint is never deleted; nothing is deleted while at most that many exist; nothing at all is deleted while two files claim the same version, because which one is newest is then ambiguous. Only the running workbook's own checkpoints are touched, never the audit JSONL, the source workbook, or a quarantined `.partial-` file. Owner decision 2026-08-28, superseding the earlier `superseded/` folder policy.
