...
`AMBIGUOUS_SUBMITTED` has two explicit fail-closed operator routes: **Resolve Existing Output** or **Recreate Image**. Resolve Existing Output is available only with a recorded submitted boundary and is read-only conversation inspection: it must match persisted original `run_id`, `job_id`, `attempt_id`, `submitted_at`, immutable pre-submit image boundary, and recorded selected attribution. The current candidate must be uniquely attributable, absent from the original boundary, visible, fully loaded, and not an input/reference image. Recreate Image requires a fresh duplicate-risk confirmation for the exact blocked job. The runner appends the prior/recreate forensic history, checkpoints that approval, then starts one deliberate new attempt with a new attempt ID. A failed or ambiguous post-submit recreate remains blocked, preserves all evidence, and exposes both explicit recovery actions again. A readiness, receiver, attachment, or other pre-submit recreate failure remains blocked and exposes only Recreate Image; it never silently resubmits. Only after attribution or recreate image persistence verification and the next immutable Result XLSX checkpoint all succeed does the job become `SAFE_COMPLETE`; otherwise it remains `AMBIGUOUS_SUBMITTED`. `DAC_WAIT_CHAT_READY` remains the authority before any later `SAFE_PENDING` job can submit.

**XLSX CONTRACT V1 UNCHANGED.** This recovery flow uses existing attempt/detector fields plus runner-owned additive `recreate_*` and `audit_chain_*` ledger fields; it introduces no source XLSX schema or config keys.
...
## Audit continuity on Resume

When Resume uses an authorized directory and `save_audit_jsonl=true`, Check Plan verifies the recorded prior audit file is present, readable, and non-empty before Run or Recreate. A missing file blocks with `RESUME_AUDIT_CHAIN_MISSING`; raw browser filesystem errors are not shown to the operator.

The only recovery is the explicit **Continue with new audit segment** action. It records runner-owned `audit_chain_status=GAP_ACKNOWLEDGED`, the expected missing filename, and acknowledgement time; then writes `AUDIT_CHAIN_GAP` as the first event in the configured audit filename and saves the next immutable Result XLSX checkpoint. Result XLSX remains authoritative, historical audit data is never fabricated, and no missing-audit bypass is automatic. Existing prior audits retain ordinary append behavior. These `audit_chain_*` fields are runner-owned additive ledger provenance, not source XLSX config.
