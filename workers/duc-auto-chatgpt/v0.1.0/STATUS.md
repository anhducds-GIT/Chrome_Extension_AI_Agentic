...
owner: claude
priority_rank: 2
next_step: "**Sổ nợ vừa được cắt: từ 38 mục mở xuống 11** (Đức chốt 10/09 — bỏ `B-43`, dọn hết mục thừa). Còn lại đúng **hai cụm**: ⑴ **GIÁC QUAN** — `B-50` panel treo hàng phút (gốc rễ), `B-58` bốn cửa nói bốn chuyện, `B-59` DOM cụt, `B-60` nút Stop nhấp nháy, `B-39` `run.status` không nói job kết thúc ra sao. Cả cụm này là **cùng một câu hỏi**: *bây giờ trên trang đang xảy ra chuyện gì*. ⑵ **CHẠY HẾT JOB** — `B-40`/`B-41` tự chữa lỗi nhà cung cấp theo ADR-0050, `B-37` `audit_durable` gộp ba nguyên nhân, `B-38` lượt ghi hết giờ vẫn có tác dụng. Ngoài ra: `B-56`⓶ chờ Đức chốt, `B-66` là việc CẮT ở vùng `_code`. **Nền chạy chuỗi coi như xong:** nút bấm `chay-chuoi.bat`/`dung-chuoi.bat`, menu chọn profile, bốn mép an toàn (`~~B-57~~` `~~B-61~~` `~~B-62~~` `~~B-63~~`), thước `do-usage-phien.mjs`, bộ nghiệm thu `nghiem-thu-artifact.mjs`. Chạy chuỗi tốn **0 usage CC**. **Pilot kế đã sẵn sàng:** `drafts/pilot-dieu-phoi/GOI-VIEC.md`."
