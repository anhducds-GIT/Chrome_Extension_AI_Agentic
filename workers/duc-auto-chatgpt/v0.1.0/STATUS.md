...
owner: claude
priority_rank: 2
next_step: "**`B-48` đã vá — nửa code xong, chờ Đức nạp lại tiện ích rồi tôi đóng `B-14` bằng một lượt 0 credit.** Trường mới `composerScope` trong `dom_probe` soi **theo phạm vi** trong `form` soạn thảo (thay vì nới nắp 40 nút mà thanh bên vẫn ăn hết), và chuỗi tổ tiên mang cả `aria-*` + `role` vì đo live nói ChatGPT **không** đặt `data-testid` lên chip. Ghim cắt-và-chạy trong `node:vm` trên DOM dựng lại đúng ca live; suite **129/129**, thử phá **10/10 đỏ, 0 lọt**. **Ba mục còn lại đều chờ Đức chốt vì đụng cổng sẵn-sàng:** `B-47` ⑵ (job ảnh cần tab hiện) · `B-49` (bỏ `!uploadIsPending()`) · và vế ⑴ của `B-47` (đổi thông điệp `NO_NEW_IMAGE` cho đúng nguyên nhân) thì AI làm được ngay, cùng họ với `B-16`."
human_action: "**Hai việc, một bấm một chốt.** ⑴ @Đức:bấm(B-48) **Nạp lại tiện ích** (`chrome://extensions` → nút nạp lại) rồi nói một câu — probe vừa được nới thêm trường soi ô soạn thảo, bản đang chạy chưa có nó. Nạp xong tôi đóng `B-14` bằng **một job chữ, 0 credit ảnh**. ⑵ **MỘT LUẬT VẬN HÀNH ĐO ĐƯỢC 09/09: job ảnh phải để tab ChatGPT HIỆN, đừng che.** Tab bị che thì ChatGPT vẫn sinh ảnh nhưng Chrome không giải mã xong bitmap, nên tiện ích **không nhận được** và job chết sau 300 giây — đúng ca `Q002` hôm nay (**mất 1 credit, ảnh nằm trong hội thoại nhưng KHÔNG được lưu**). Job **chữ** thì che vẫn xong bình thường. @Đức:chốt(B-47) Có cho cổng sẵn-sàng **từ chối gửi job ảnh khi tab đang bị che** không (tôi khuyên CÓ)? @Đức:chốt(B-49) Và có cho bỏ điều kiện `!uploadIsPending()` khỏi vòng chờ gắn ảnh không (nó đo sai thứ nó khai)? Hai câu này đụng cổng an toàn nên tôi không tự làm."
