...
owner: claude
priority_rank: 2
next_step: "**ĐỌC `## ROADMAP MVP` ở cuối `BACKLOG.md` TRƯỚC KHI LÀM GÌ.** `~~M0~~` và `~~M3~~` **đã đạt 10/09**: Đức bấm một nút, rồi **13 ảnh, 13/13 thành công, 0 hỏng, 0 lần thử lại**, tên file đúng cả 13, mã băm khác nhau cả 13. Còn lại: **M1** — một lượt việc THẬT do **Đức ra đề** (đây là chặng duy nhất tôi không tự làm thay được, vì điều kiện đạt là *Đức dùng được ảnh vào việc thật*). **M2:** `B-47`⑴ và `B-51` tôi tự làm · `B-49` **cần Đức chốt**. Mở thêm 10/09: `B-54` (`landed_as_requested` luôn `unknown` trên đường thư mục — mất một bảo chứng khi chạy `uniquify`). `B-50` **đã loại được một nửa nghi phạm** bằng phép đo lượt sạch, nghi phạm còn lại là lượt ghi checkpoint XLSX mỗi mutation."
human_action: "**Chỉ còn MỘT việc, và là việc duy nhất tôi không làm thay được: cho tôi một đề bài THẬT.** @Đức:chốt(M1) Vài ảnh Đức thật sự cần dùng, không phải prompt tôi bịa để đo — điều kiện đạt của M1 là Đức **dùng được** ảnh đó. @Đức:chốt(B-49) Và một câu cần Đức quyết: bỏ điều kiện đo nhầm **và** đổi vòng chờ gắn ảnh sang đếm theo TÊN FILE — hai vế cùng một hàm, làm nửa vời là mở cổng gửi sớm. *Cấp quyền thư mục vẫn là một cú bấm sau MỖI lần nạp lại tiện ích — Chrome bắt buộc thế, không rút xuống 0 được.*"
