...
owner: claude
priority_rank: 2
next_step: "**Chạy tiếp được rồi** (`~~B-74~~`, Đức nêu 12/09): cuối mỗi lượt `chay-chuoi.bat` hỏi `[t]` chạy tiếp · `[m]` chạy mới cùng thông số · `[g]` gõ lại; và thông số được nhớ sang cả lần mở cửa sổ sau. `--tiep` đọc nhật ký để nối từ lượt gửi cuối và **trừ** số vòng đã gửi vào trần — chạy lại từ số không trên một hội thoại đang dở sẽ **gửi lại đúng prompt đã gửi**. **Trước đó, `~~B-73~~`:** tên chuỗi có **dấu cách** làm vỡ khoá gửi — mọi lượt gửi trả `INVALID_ENVELOPE` và chuỗi chết ở vòng 1, trong khi nhật ký lại đổ cho trang. Cùng họ: hai dòng `echo` không đóng ngoặc khiến `&` trong tên **chạy `fill` như một lệnh**. Cả hai đã vá. **Sổ nợ 2**: `B-50` · `B-58`, cả hai chờ bệnh tái phát."
