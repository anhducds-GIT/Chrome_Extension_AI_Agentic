...
owner: claude
priority_rank: 2
next_step: "**ĐỌC `## ROADMAP MVP` ở cuối `BACKLOG.md` TRƯỚC KHI LÀM GÌ.** `~~M0~~` và `~~M3~~` **đạt 10/09**: Đức bấm một nút, rồi **13 ảnh, 13/13 thành công, 0 hỏng, 0 lần thử lại**, 13 tên đúng, 13 mã băm khác nhau. `~~B-49~~` **đã vá 10/09** (Đức chốt cả hai vế): cổng gắn ảnh nay đối chiếu **TÊN FILE** trên chip thay vì đếm, và thôi hỏi `uploadPending` — suite 132/132, thử phá 10/10 đỏ, **nhưng CHƯA nghiệm thu live**, cần một job có ảnh mẫu. Còn lại: **M1** — một lượt việc THẬT do **Đức ra đề** (Đức nói làm sau). **M2:** `B-47`⑴ và `B-51` tôi tự làm. `B-54` mới 10/09. `B-50` đã loại được một nửa nghi phạm; còn lại là lượt ghi checkpoint XLSX mỗi mutation."
human_action: "**Nạp lại tiện ích để bản vá `~~B-49~~` vào hiệu lực, rồi cho tôi một job có ảnh mẫu** — đó là thứ duy nhất suite không thay được. @Đức:bấm(B-49) Còn `M1` (đề bài thật) Đức đã nói để sau, tôi không hỏi lại. *Cấp quyền thư mục vẫn là một cú bấm sau MỖI lần nạp lại — Chrome bắt buộc, không rút xuống 0 được.*"
