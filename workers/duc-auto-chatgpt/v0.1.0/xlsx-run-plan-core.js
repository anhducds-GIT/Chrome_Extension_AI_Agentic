(() => {
  ...
  const SUPPORTED = new Set([
    "timeout_sec", "max_retries", "delay_min_sec", "delay_max_sec", "delay_sec", "safety_cooldown_sec", "max_input_images", "continue_on_error", "rerun_done", "checkpoint_interval_jobs", "checkpoint_retention", "output_folder",
  ...
  function validate(config = {}, jobs = [], runner, output) {
    ...
    const runtimeRaw = Object.fromEntries([...SUPPORTED].filter((key) => key in raw && ["timeout_sec", "max_retries", "delay_min_sec", "delay_max_sec", "delay_sec", "safety_cooldown_sec", "max_input_images", "continue_on_error", "rerun_done", "checkpoint_interval_jobs", "checkpoint_retention", "output_folder"].includes(key)).map((key) => [key, raw[key]]));
    let runtime = null;
