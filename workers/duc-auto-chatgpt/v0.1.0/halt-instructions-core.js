...
(() => {
  ...
  const HALT_GROUPS = Object.freeze([
    ...
    Object.freeze({
      ...
      retry: "Depends -- a job that only timed out waiting for the page retries; a job that HALTED here is kept, not resent (Tuỳ -- job chỉ hết giờ chờ trang thì thử lại; job ĐÃ DỪNG HẲN ở đây được giữ nguyên, không gửi lại)",
      meaning: "The output was already written and verified, but ChatGPT didn't return to a ready state within the allowed time. Readiness is about the NEXT job -- it says nothing about this one. (Kết quả đã ghi và đã xác minh, nhưng ChatGPT không trở lại trạng thái sẵn sàng trong thời gian cho phép. Readiness nói về job KẾ TIẾP -- nó không nói gì về job này.)",
      action: "Nothing is lost and nothing gets resent. Since 2026-09-14 (Đức chốt): a job that HALTED here keeps its verified output -- on Continue Run the recovery plan reads it as SAFE_COMPLETE and skips it, so you never re-setup the prompt and never spend a second generation on an answer already on disk. Three conditions must all hold, or it still stops for review: the job really halted, its output passes the saved-attribution check, and its response_sha256 still re-hashes to the same value (an answer cell edited by hand is refused). A job that merely timed out mid-queue still auto-retries with a fresh prompt as before. If the page keeps failing to go idle, raise the Timeout setting -- one number covers both waiting for the answer and waiting for the page, and it now goes up to 3600 seconds. (Không mất gì và không gửi lại gì. Từ 14/09 (Đức chốt): job ĐÃ DỪNG HẲN ở đây vẫn giữ kết quả đã xác minh -- lúc Continue Run, kế hoạch phục hồi đọc nó là SAFE_COMPLETE và bỏ qua, nên anh không phải setup lại prompt và không tốn thêm một lượt sinh cho câu trả lời đã nằm trên đĩa. Phải đủ CẢ BA điều kiện, thiếu một là vẫn dừng để soát: job thật sự đã dừng hẳn, kết quả qua được phép kiểm quy thuộc, và response_sha256 băm lại vẫn khớp (ô câu trả lời bị sửa tay thì bị từ chối). Job chỉ hết giờ giữa hàng đợi thì vẫn tự thử lại bằng một prompt mới như cũ. Nếu trang cứ không chịu rảnh, hãy nâng ô Timeout -- một con số nuôi cả chặng chờ câu trả lời lẫn chặng chờ trang, và nay nới tới 3600 giây.)"
    }),
    Object.freeze({
