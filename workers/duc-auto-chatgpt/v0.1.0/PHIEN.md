...
- next_step: "**BỘ CHẠY CHUỖI ĐÃ CHẠY LẠI ĐƯỢC** — `~~B-73~~` vá 12/09: tên chuỗi có **dấu cách** làm vỡ khoá gửi. Tên `HNX audit & fill` ghép thành `HNX audit & fill-v1`, mà host đòi `/^[\x21-\x7e]{8,128}$/` — **dấu cách không nằm trong đó** — nên mọi lượt gửi trả `INVALID_ENVELOPE` và chuỗi chết ở vòng 1. Nhật ký lại kết thành *"hai lượt gửi đều không thấy trong hội thoại"* — đọc y như trang hỏng. Tên tiếng Việt có dấu vỡ y hệt. Nay khoá được rút an toàn + vân tay tên gốc (chống hai chuỗi đụng khoá), và công cụ chặn ngay tại cửa kèm **tên đích danh ký tự hỏng**. **Sổ nợ 2**: `B-50` · `B-58`, cả hai chờ bệnh tái phát."
