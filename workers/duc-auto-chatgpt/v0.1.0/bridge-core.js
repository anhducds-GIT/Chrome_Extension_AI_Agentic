(() => {
  ...
  const METHOD_ENTRIES = [
    ...
    registryEntry({ name: "system.ping", context: "router", read_only: true, approval: "none", deadline_ms: 30000, description: "Report fresh extension, executor, ChatGPT, and workbook availability. chatgpt also carries url and conversation_id - the address the provider tab is on right now, and the conversation id parsed out of it, or null when the tab is not on a conversation. These two ship HERE rather than on chat.read because chat.read refuses off a conversation page with WRONG_SURFACE, which makes it useless for the one question an attaching caller has to answer first: which conversation am I about to type into. A caller that wants a specific conversation compares conversation_id, never the raw url - the address gains and loses query strings on its own, and a brand-new chat sits at the site root with no id at all until its first message.", params_schema: {}, params_validator: validateEmptyParams }),
    ...
    registryEntry({ name: "chat.read", context: "executor", read_only: true, approval: "none", deadline_ms: 30000, description: "Read the newest conversation turns on the provider tab as text, oldest first inside the returned slice, each with role, id, character count and its own truncation flag. Strictly read-only: never clicks, types, or moves focus, and allowed while a run is live because reading disturbs nothing. Refused with WRONG_SURFACE off a conversation page, since reading a launcher page returns zero turns and looks exactly like an empty conversation. Reports NO_TURNS_MATCHED plus the data-attribute names actually present when the turn selector has rotted, so a dead selector is rebuilt from evidence instead of guessed. Also reports generating: whether the page is still producing this answer, read from the same stop-button probe the runner uses. It ships on THIS read rather than a second RPC because the two would race, and because every caller that lacked it invented a wrong proxy instead: the panel's busy flag describes the panel and not the answer, and a character count that stops growing means the model is running a tool, not that it has finished - a measured 3-minute plateau mid-answer was read as a dead turn and the prompt was resent twice. Read it narrowly: generating states only whether a Stop control was present during THIS read. True is a reliable keep-waiting. False is NOT a certificate that the answer is complete - measured on 2026-09-10, three turns out of four reported generating false while the live DOM still held only 48 to 173 characters of an answer that a reload then showed in full at 2934 to 3450 characters. So a caller that finds no expected content on a generating-false read must reload the tab once and read again before concluding the content is absent. The two caps are also bounded together: limit x max_chars_per_turn may not exceed 200000 characters, because either cap at its own maximum is fine and the product would overflow the 1 MB envelope.", params_schema: { limit: "integer:1..50", max_chars_per_turn: "integer:200..40000", _total: "limit * max_chars_per_turn <= 200000" }, params_validator: validateChatRead }),
