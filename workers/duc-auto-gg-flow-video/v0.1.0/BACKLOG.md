...
- **F-28 · SỬA SỔ + XÁC MINH LẠI** · 2026-09-07 · lane `claude-flow-f28-f33`. Dòng đóng ở trên có **một câu sai**: *"Chuỗi `chatgpt.com` cũng rời khỏi `tests/` của gói Flow theo."* Đo lại bằng `grep -rn "chatgpt\.com" tests/`: còn **3 file, 6 lượt**. Xoá file chết chỉ dọn được một chỗ. Vế hai của `Đóng khi:` thì ĐẠT thật — đột biến vào `syncZoomState` của `sidepanel.js`, **4/4 bị bắt** (bỏ lệnh bật lại nút · ép nút không bao giờ sáng · không dọn lý do cũ trên đường tốt · bỏ `lockZoomButtons` khi không có tab). Một con thứ năm (bỏ cửa sớm `if (!zoomButtons.length) return`) **không đỏ** và **không tính**: nó là đột biến tương đương — `forEach` trên danh sách rỗng vô hại, cửa đó chỉ tiết kiệm một lượt hỏi tab. Ba chỗ còn chuỗi, phân loại từng chỗ một chứ không thay hàng loạt: ① `tests/worker-api-smoke.mjs` — **di sản fork thật**, 4 lượt `https://chatgpt.com/generated.png` chỉ là fixture cho một cổng **chỉ đọc scheme** (`downloadGeneratedImage` khớp `^https://` hoặc `^data:image/`, host không bao giờ được đọc). Đã đổi sang host của nhánh này, kèm một chú thích nói rõ **đây không phải địa chỉ đã đo**. ② `tests/artifact-integrity-smoke.mjs` — **bản ghi lịch sử, cấm rebrand**: câu đó kể lỗi được ĐO ở đâu (Pilot-05/06 chạy trên ChatGPT, và tên artifact trong chính file vẫn là `Duc-Auto-ChatGPT-Pilot-05__…`). Đổi nó thành host Flow là **làm sai xuất xứ**. Đã ghi cảnh báo ngay tại dòng đó cho phiên sau. ③ `tests/zoom-control-smoke.mjs` — **cố ý, và nay có răng**: chuỗi nằm trong chú thích kể lại khẳng định sai của file chết, và tôi thêm **2 ca cấm** (`https://chatgpt.com/` · `https://chatgpt.com/c/abc-123`) vào khối "Ca E". Đột biến nhét `"chatgpt.com"` vào `ORIGIN.hosts` của `provider-adapter.js`: **bị bắt** (trước khi thêm ca này thì đột biến đó **không ai canh** — danh sách cũ không có host ChatGPT nào). Cũng sửa một câu đã lạc hậu trong chú thích file đó (*"File chết CHƯA XOÁ"* — nó đã bị xoá ở `b7b877b`).
- **F-28 · ĐIỀU KIỆN ĐÓNG VIẾT QUÁ RỘNG, sửa lại cho đúng** · 2026-09-07 · lane `claude-flow-f28-f33`. Vế *"không còn chuỗi `chatgpt.com` nào trong test của gói này"* là **không thể đạt và không nên đạt**: một phép kiểm cấm-chuỗi buộc phải chứa chính chuỗi bị cấm, và một bản ghi lịch sử buộc phải giữ tên nơi đã đo. Điều kiện đúng, viết lại: **mỗi lượt còn lại của `chatgpt.com` trong `tests/` phải thuộc đúng một trong hai loại — (a) ca CẤM có đột biến chứng minh nó đỏ khi chuỗi đó được nhận, hoặc (b) bản ghi lịch sử về nơi lỗi đã được ĐO, có nhãn ngay tại dòng đó — và không lượt nào là fixture tuỳ ý.** Đo hiện tại (`grep -ro "chatgpt\.com" tests/ | wc -l`): **6 lượt / 2 file** — `artifact-integrity-smoke.mjs` 1 (loại b), `zoom-control-smoke.mjs` 5 (loại a + chú thích của chính ca đó). **0 lượt** là fixture tuỳ ý. Trước phiên này: 6 lượt / 3 file. Suite **101 → 101** (không thêm file, thêm 2 ca vào file có sẵn). Mục F-28 **đóng, không mở lại**.
- **ĐÓNG F-33** · 2026-09-07 · lane `claude-flow-f28-f33` · **đột biến 10/10 bị bắt, suite 101 → 101.** Nợ còn lại của mục là *"đường nhà mới chưa có test hành vi nào"* — nay có. ⑴ **Gốc bệnh dọn hẳn, không vá quanh:** cả hai harness bỏ cách giả lập `querySelectorAll` bằng **so bằng chữ**, thay bằng một bộ đọc selector thật (tách theo dấu phẩy rồi đọc mỗi phần như chuỗi thẻ-tổ-hậu-duệ). Cách cũ im lặng trả rỗng ngay khi adapter thêm nhánh thứ hai; cách mới **thấy được** cả `flow-video-tile img` lẫn `flow-image-tile img` và phân biệt chúng bằng **thẻ bọc**, đúng như trình duyệt. ⑵ Sân khấu nhà mới dựng đúng hình dạng đã ĐO (`evidence/F31-dom-probe-flow-google-com-20260906.json`): không thẻ `<video>` nào, video là `<img>` địa chỉ `flow.google.com/asb/<mã>`, và **ảnh tải lên dùng đúng cùng dạng địa chỉ** chỉ khác thẻ bọc. Tile nhà mới còn được khai đủ ba món mà bộ dò ảnh gọi trên mọi `<img>` (`closest`/`alt`/`getAttribute`) — thiếu chúng thì harness vỡ ngay, và đó là bằng chứng nó đang chạy đúng đường thật. ⑶ Bốn ca hành vi mới ở `tests/flow-video-safety-behavior.mjs` (đường tốt · ảnh tải lên hiện giữa đường · hai tile mới → `OUTPUT_AMBIGUOUS` · tile không quy gán được), và `tests/content-abort-race-behavior.mjs` chuyển sang nhà mới (nhà cũ vẫn được canh ở file kia). ⑷ **Đột biến, kể tên từng con:** `videoSelector` mất nhánh nhà mới · nới thành `video, img` · neo sai thẻ bọc · bỏ nhánh `<video>` của nhà cũ (ca hồi quy) · `videoIdFromSrc` mất nhánh `/asb/` · lấy sai đoạn của `/asb/<mã>` · bỏ cổng host · `videoSource` bỏ `currentSrc` · `videoCandidates` bỏ lọc id rỗng · `waitForVideoCompletion` bỏ luật hai-ứng-viên-thì-không-nhận. **Cả 10 đỏ.** ⑸ **Một con thoát ở lượt đầu, và nó chỉ đúng vào lỗ:** bỏ `filter((candidate) => candidate.id)` mà suite vẫn XANH — vì mọi tile trong sân khấu đều quy gán được, nên cái lọc đó không có gì để lọc. Đã thêm ca ⑷ (tile đúng thẻ bọc, địa chỉ rỗng) rồi phá lại: đỏ. Cùng họ với hai bẫy đã ghi trong sổ — sân khấu thiếu đúng trạng thái mà phép kiểm phải cắn thì phép kiểm mất răng ở đúng chỗ đó. Ca này **cố ý khai rõ nó không phải trạng thái đã đo của Flow**, nó là ca ghim cho chính cái lọc.
- **F-33 · PHÁT SINH, không tự làm** · 2026-09-07 · lane `claude-flow-f28-f33`. Bộ đọc selector của harness mới **chỉ hiểu chuỗi thẻ** (`flow-video-tile img`); nó **không** hiểu class, thuộc tính, hay `>`. Hôm nay đủ vì `videoSelector` chỉ dùng tên thẻ. Nhưng nếu ai thêm một nhánh dạng `[data-…]` hay `.class` vào `videoSelector` thì bộ đọc này **im lặng trả rỗng** — đúng lại cái bệnh vừa chữa, chỉ ở tầng sâu hơn một bậc. **đóng khi:** hoặc `videoSelector` vẫn thuần tên thẻ và có một phép kiểm ghim điều đó (đỏ khi ai thêm `.`/`[`/`>` vào), hoặc bộ đọc của harness được nâng để hiểu đúng phần cú pháp đã được dùng.
