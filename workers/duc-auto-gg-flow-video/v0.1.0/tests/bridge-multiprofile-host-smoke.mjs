...
function instanceAuth(instanceId, label) {
  return { type: "auth", role: "extension", token, instance: { schema_version: 1, instance_id: instanceId, label, worker: "duc-auto-gg-flow-video", extension_version: "0.1.0" } };
}
...
extA.send(instanceAuth(A_ID, "kaito"));
...
extC.send(instanceAuth(C_ID, "kaito"));
...
extA2.send(instanceAuth(A_ID, "kaito"));
...
extLegacy.send({ type: "auth", role: "extension", token });
...
extBad.send({ type: "auth", role: "extension", token, instance: { schema_version: 2, instance_id: "b".repeat(12) } });
