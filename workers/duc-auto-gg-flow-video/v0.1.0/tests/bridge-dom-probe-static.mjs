...
assert.match(content, /const FLOW_RUNTIME_CONTRACT = "flow04-composer-cluster-submit-v2";/, "content runtime contract is a stable exact constant");
...
assert.match(probeBlock, /in_composer_cluster:/, "every reported visible button proves whether it belongs to the composer control cluster");
assert.doesNotMatch(probeBlock, /in_composer_form\s*:/, "the disproved form-ownership field name must not survive in the payload");
