...
const pairingPath = "C:\\WORKING ZONE\\Chrome Extension Bridge\\duc-auto-gg-flow-video\\duc-auto-gg-flow-video-bridge-pairing-v1.json";
...
assert.match(html, /id="bridgePairingPathCopyBtn"/, "Bridge screen exposes the one-click copy control");
const buttonStart = html.indexOf('<button id="bridgePairingPathCopyBtn"');
...
assert.match(script, /bridgePairingPathCopyBtn\?\.addEventListener\("click", \(\) => copyBridgePairingPath\(els\.bridgePairingPathCopyBtn\)\)/, "copy control is wired to its click handler");
