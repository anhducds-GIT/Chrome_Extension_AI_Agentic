...
**2.603 token.** Đừng nạp `AGENTS.md` hay file nhật ký này lúc mở — chúng là **nguồn** của
`PHIEN.md`, mở khi cần đào sâu.

`## Luật vàng` 2921 → 2129 ký tự. **Không luật nào bị xoá** — phần bỏ là phần đã có nguyên văn
ở lõi dùng chung và `workers/_shared/AGENTS.md`. Giữ nguyên: trần credit suy từ chip · `MAX_TRIAL_JOBS = 7` là trần tuyệt đối · không retry khi nghi đã trừ credit · ranh giới *"fix nhỏ"* vẫn **chờ Đức**.
