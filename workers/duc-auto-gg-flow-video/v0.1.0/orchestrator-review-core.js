(() => {
  ...
  function checklist({ workbook, prepared, diagnostics, outputSettings, output, settings }) {
    ...
    add("chatgpt", "Flow connection", chatFindings.filter((finding) => finding.severity !== "OK"), diagnostics ? (chatFindings.some((finding) => finding.code === "CHATGPT_OK") ? "OK" : "BLOCKER") : "WARNING", chatFindings.find((finding) => finding.code === "CHATGPT_OK")?.message || chatFindings[0]?.message || "Check plan");
    return sections;
  }
