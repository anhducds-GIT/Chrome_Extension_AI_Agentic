(() => {
  ...
  function safeRelativeFolder(value, fallback = "Duc Auto GG Flow") {
  ...
  function fromWorkbook(config, workbookName) {
    ...
    const downloads = downloadsLocation(config?.output_downloads_subfolder || config?.output_folder || "Duc Auto GG Flow");
    ...
    const result = !separate ? { kind: "same_as_image" } : resultMode === "profile"
      ...
      : downloadsLocation(config?.result_downloads_subfolder || config?.output_downloads_subfolder || config?.output_folder || "Duc Auto GG Flow");
    return {
