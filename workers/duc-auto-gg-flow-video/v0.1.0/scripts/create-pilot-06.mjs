/* Builds the Pilot-06 acceptance workbook.

   Pilot-06 is the post-fix verification run for the 2026-08-22 repairs. It
   mirrors the Pilot-05 setup but lives in its own folder so the Pilot-05
   artifacts stay intact as the evidence for the defects being verified.

   Deliberate choices:
   - no reference images, so the operator selects one file and nothing else;
   - three short jobs, so a full run plus a continuation stays quick;
   - collision_policy=overwrite, so a first write must now report `written`;
   - a mixed-case audit filename, so filename case must survive into provenance.

   Run: node workers/duc-auto-chatgpt/v0.1.0/scripts/create-pilot-06.mjs
*/
...
const config = [
  ...
  ["output_profile_id", "pilot-06"],
  ["output_folder_hint", "C:\\WORKING ZONE\\Chrome_Extension_AI_Agentic\\workers\\duc-auto-chatgpt\\v0.1.0\\pilot-06"],
  ...
  ["result_filename_pattern", "Duc-Auto-ChatGPT-Pilot-06__results__v{version}.xlsx"],
  ["audit_filename", "Duc-Auto-ChatGPT-Pilot-06__audit.jsonl"]
];

const jobs = [
  ["id", "prompt"],
  ["P06-A", "A single orange ceramic teapot on a plain white table, soft daylight, minimal product photo."],
  ["P06-B", "A small brass pocket watch lying open on dark blue velvet, warm studio light."],
  ["P06-C", "One green paper origami crane on a plain grey background, soft even lighting."]
];
...
const folder = new URL("../pilot-06/", import.meta.url);
fs.mkdirSync(folder, { recursive: true });
const target = new URL("Duc-Auto-ChatGPT-Pilot-06.xlsx", folder);
if (fs.existsSync(target)) throw new Error("pilot-06 workbook already exists; delete it deliberately before regenerating.");
