/* Pure decision logic for the Gemini content receiver, ported verbatim from
   the proven duc-auto-gemini/v0.1.0 content-decision-core.js (19 pinned
   behaviors). Every branch is DOM-free so tests can exercise the exact
   fail-closed rules the page code runs:
   - blockingFailure / guardedAction: abort, security and quota blockers stop
     any click before it happens (submit-once, fail-closed).
   - exposeFileInput: Gemini's file input is transient -- it exists only while
     the "Upload & tools" menu is open. Trigger click, wait, optional menu-item
     click, wait; every click is guarded.
   - addedSince / attachmentReady: attachment arrival counts unique NEW nodes
     inside the composer scope. A placeholder replaced in place by the finished
     thumbnail is one arrival; summed selector totals would miss it entirely.
   - sendReady / clickSend: Send fires only when found, enabled, and no blocker
     or operator stop is active. */
(() => {
  "use strict";
  ...
  const api = Object.freeze({ blockingFailure, attachmentReady, addedSince, sendReady, exposeFileInput, guardedAction, clickSend });
  const scope = (typeof window !== "undefined" ? window : globalThis);
  scope.DacContentDecision = api;
  // Alias kept so v0.1.0-derived tooling and docs keep resolving; same object.
  scope.DagContentDecisionCore = api;
})();
