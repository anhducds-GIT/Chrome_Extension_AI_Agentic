(() => {
  ...
  function validateRunTrial(raw) {
    ...
    if (!Array.isArray(params.job_ids) || params.job_ids.length < 1 || params.job_ids.length > 30) {
      invalidParams("params.job_ids", "expected 1-30 job ids (an AI-run chain covers the owner's real 20-30 image workload; beyond 30 stays owner-clicked)");
    }
  ...
  const METHOD_ENTRIES = [
    ...
    // FLOW-01 evidence scaffold (2026-08-27): the ONLY interaction primitive of
    ...
    registryEntry({ name: "diagnostics.evidence_submit", context: "executor", read_only: false, approval: "none", deadline_ms: 30000, description: "FLOW-01 scaffold: type one prompt into the Flow composer and click Create, once; content-side hard cap 3 per page load (owner free-credit budget). Evidence: evidence/F1-snapshot-1-idle-20260827.json.", params_schema: { prompt: "string" }, params_validator: (raw) => { const params = assertPlainObject(raw, "params"); rejectUnknown(params, ["prompt"], "params"); return { prompt: stringValue(params.prompt, "params.prompt", { min: 1, max: 2000 }) }; } }),
