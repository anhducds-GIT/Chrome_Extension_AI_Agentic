(() => {
  ...
  const GLOSSARY = Object.freeze([
    ...
    ["SETUP", "Safety cooldown", "Khoảng giữ thêm sau khi Flow vừa đạt trạng thái READY. Có thể nhập một số cố định hoặc dải như 6-9; extension chọn một số nguyên cho mỗi readiness gate, chờ đủ thời gian rồi kiểm tra READY lần nữa. Khoảng này khác với Inter-job delay."],
    ...
    ["RUN", "Waiting for Flow ready", "Extension đang xác minh trình soạn thảo ở trạng thái rảnh. Việc bộ đếm thời gian kết thúc không tự nó cho phép gửi prompt."],
    ["RUN", "Response / output detection", "Extension theo dõi ranh giới của một lần thực hiện để xác định đầu ra mới có thể quy thuộc; video cũ hoặc ảnh tham chiếu không được xem là đầu ra mới."],
    ...
    ["OUTPUT", "Artifact", "Một tệp đầu ra do extension quản lý: video đã tạo, Result XLSX hoặc Audit JSONL."],
    ["OUTPUT", "Jobs completed", "Số job đã hoàn tất phần tạo video. Ví dụ 9 / 9 completed chỉ xác nhận chuỗi render đã xong; con số này không tự chứng minh các artifact đã được lưu thành công."],
    ...
    ["OUTPUT", "Artifact persistence failed", "Phần tạo video có thể đã hoàn tất, nhưng ít nhất một artifact không ghi được hoặc không vượt qua bước mở lại để xác minh. Xem bảng Run Artifacts để biết Images, Result XLSX hay Audit JSONL bị lỗi; giữ nguyên output trong Flow cho đến khi đã bảo toàn tệp."],
    ["OUTPUT", "Detected not downloaded", "Đã xác định được output mới, nhưng chức năng tải video xuống đang tắt; vì vậy không có tệp video nào được lưu."],
    ["OUTPUT", "Result XLSX", "Checkpoint dạng Excel ghi trạng thái job, tên tệp và bằng chứng tiếp tục lần chạy. Đây không phải là video output."],
    ["OUTPUT", "Audit JSONL", "Nhật ký kỹ thuật theo từng sự kiện để truy vết lần chạy; tệp này khác với Result XLSX và video đã tạo."],
