...
| Job dừng `PRE_SUBMIT` với `WRONG_GENERATION_MODE` (không thấy nút Video, hoặc bấm rồi mà mode không đổi) | **ĐÃ GIẢI 02/09 — ĐỪNG NHỜ ĐỨC ĐẶT TAY NỮA.** Chẩn đoán cũ ~~"`element.click()` không ăn với nhóm nút cấu hình của Flow (class `flow_tab_slider_trigger`), nên chuyển mode phải do người"~~ **đã hết hạn**. Đúng một nửa: `.click()` trần thật sự không mở được bảng (đo 28/08, hai lượt). Nhưng runner nay dùng `pressFlowControl()` — bắn đủ chuỗi `pointerdown` → `mousedown` → `pointerup` → `mouseup` → `click` — và nó **mở được bảng lẫn đổi được mode thật**, chứng minh live trên trang thật, 0 credit (**F-14**, `evidence/F26-KET-QUA-luot1.md`) | Runner **tự chuyển mode**, không cần Đức chạm vào. Còn gặp lỗi này thì nguyên nhân nằm ở chỗ khác, theo thứ tự đáng ngờ: ① nhãn Image mode trên giao diện của Đức là biến thể chưa đo (**F-11** — nhãn nhận theo cấu trúc `emoji + tên model + crop_* + x{n}`, biến thể lạ sẽ trả `unknown`); ② `.js` sửa rồi mà chưa reload extension. Chẩn đoán bằng `diagnostics.mode_probe` — 0 credit, không bao giờ bấm một tuỳ chọn mode nào. Mọi đường thoát đều 0 credit, `retry_count=0` |
| Chip cấu hình đang để `x2`/`x3`/`x4` | Flow sẽ sinh nhiều video một lượt. Luật gán chỉ nhận **đúng 1 id mới**, nhiều hơn thì trả `OUTPUT_AMBIGUOUS` và không nhận cái nào — **credit vẫn bị tiêu** | **ĐÃ TỰ ĐỘNG 02/09 — ĐỪNG NHỜ ĐỨC ĐỔI CHIP NỮA.** Câu cũ ~~"Runner chưa tự kiểm việc này"~~ hết hạn: **F-15** cho runner đọc `x{n}` từ nhãn chip và **từ chối trước khi gõ, trước mọi cú bấm** nếu không phải `x1` (nên mọi đường thoát đều 0 credit); **F-26** cho runner **tự mở bảng, tự bấm `x1`, rồi ĐỌC LẠI CHIP** để kết luận — không tin cú bấm. Cổng từ chối của F-15 vẫn đứng làm lớp cuối. Trường `output_chip` trong sổ cái (`label_before` · `count_before` · `fix_attempted` · `count_after` · `fixed`) ghi lại mọi lần runner tự đổi cấu hình của Đức |
