 ...
 *
 * **File này bị so từng byte giữa hai gói.** Mọi thứ riêng của một gói — số lệnh, tên miền,
 * tên gói, câu bảo bật máy chủ — đi vào bằng THAM SỐ, không gõ ở đây.
 *
...
/* Số method KHÔNG gõ ở đây. File này được chép nguyên văn sang gói kia và bị so từng byte, mà
 * hai gói có hai con số khác nhau — gõ cứng là một trong hai bản khai sai. Nơi gọi truyền vào,
 * và nơi gọi lấy nó từ `capabilities()` thật chứ không gõ tay. */

/* Trạng thái dây → một câu cho người đọc, và một câu bảo làm gì.
 *
 * Hai câu *"làm gì tiếp"* đi vào bằng THAM SỐ chứ không gõ ở đây: mỗi gói bật máy chủ bằng
 * một lệnh khác và sinh tệp ghép cặp bằng một tên gói khác, mà file này thì bị so từng byte
 * giữa hai gói. Một câu gõ cứng ở đây là một lời chỉ dẫn SAI ở một trong hai bản — và sai
 * đúng lúc người ta đang hỏng và đi tìm câu chỉ dẫn. */
function cuaDay(tenGoi, cachBatMayChu) {
  return {
    connected: { dat: true, noi: "đã nối máy chủ Bridge trên máy này", lamGi: null },
    disconnected: {
      dat: false,
      noi: "chưa nối được máy chủ Bridge",
      lamGi: `Máy chủ Bridge chưa chạy. ${cachBatMayChu} rồi bấm lại nút này.`
    },
    unpaired: {
      dat: false,
      noi: "chưa chọn tệp ghép cặp",
      lamGi: "Ở khối 'Kết nối Bridge' ngay trên, chọn tệp ghép cặp mà bộ cài đã tạo. " +
             `Chưa có tệp đó thì chạy \`tao-tep-ghep-cap.mjs --goi ${tenGoi}\` trước.`
    }
  };
}
...
export async function kiemNhanh(do_, tuyChon = {}) {
  /* `mien` có thể là `null`: gói mở `<all_urls>` thì không có "trang của tôi" nào để đếm. */
  /* Ba tham số BẮT BUỘC, kiểm NGAY ĐẦU — trước cả phép dò đầu tiên. Thiếu một trong ba là
   * lỗi của người nối dây, không phải một bước hỏng của bản cài — trộn hai loại ấy vào nhau
   * thì Đức đọc được một dòng đỏ bảo anh ấy đi bật máy chủ, trong khi máy chủ vẫn chạy. */
  if (!Number.isInteger(tuyChon.soMethod)) {
    throw new Error("kiemNhanh: thiếu `soMethod` — số lệnh của gói, lấy từ `capabilities()` chứ đừng gõ tay.");
  }
  if (typeof tuyChon.tenGoi !== "string" || typeof tuyChon.cachBatMayChu !== "string") {
    throw new Error("kiemNhanh: thiếu `tenGoi` hoặc `cachBatMayChu` — hai câu chỉ dẫn này riêng của từng gói.");
  }
  const mien = tuyChon.mien ?? null;
  const buoc = [];
  ...
  const cua = cuaDay(tuyChon.tenGoi, tuyChon.cachBatMayChu)[day] || {
  ...
  const dungSo = ms.length === tuyChon.soMethod;
  them("② Từ vựng", dungSo, `${ms.length} lệnh · ${ghi} lệnh ghi · ${ms.length - ghi} lệnh chỉ đọc`,
    dungSo ? null : `Chờ ${tuyChon.soMethod} lệnh mà thấy ${ms.length} — Chrome đang chạy một bản KHÁC bản trong thư mục này. Vào chrome://extensions bấm nạp lại đúng tiện ích đó.`);
  ...
  const cuaMien = mien ? web.filter((t) => { try { return new URL(t.url).hostname === mien; } catch { return false; } }) : [];
  them("③ Nhìn thấy tab", web.length > 0,
    mien
      ? `${web.length} tab web${cuaMien.length ? `, trong đó ${cuaMien.length} tab ${mien}` : `, KHÔNG có tab ${mien} nào`}`
      : `${web.length} tab web trong ${(tab || []).length} mục tiêu`,
   ...
   * Ưu tiên tab của miền đã khai: đó là tab gói sinh ra để làm việc, và nếu chỉ tab ấy hỏng thì ba
   * bước trên vẫn xanh trong khi việc thật vẫn không chạy. */
  const dich = (cuaMien[0] || web[0]);
  let khop = null, noi = "";
  ...
  them("⑤ Công tắc ghi", true,
    ct?.enabled
      ...
      : "ĐANG ĐÓNG — đúng mặc định. Muốn tiện ích bấm và gõ thì bật công tắc ở đầu bảng.",
    null);
  return ket();
}
