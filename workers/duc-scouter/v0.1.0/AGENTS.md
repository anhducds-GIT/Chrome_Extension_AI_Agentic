...
| `../../_shared/goi-bridge/` | **LÕI DÙNG CHUNG** — bên gọi Bridge từ dòng lệnh. `pilots/trang-thu-cham/scripts/goi-bridge.mjs` chỉ còn là cái vỏ năm dòng khai danh tính Scouter (`duc-scouter.bridge` · `SCOUTER_GHEP` · `SCOUTER_GHE`) |
...
| `scripts/do-doc-lai.mjs` | **PHÉP ĐO ③** — gõ xong thì đọc lại được bằng đường nào, và xoá xong ô có rỗng thật không, trên **năm** loại ô nhập (`S1` + `S-27`). Ô `readonly` là hàng DUY NHẤT chờ một nhánh ĐỎ — bỏ nó đi thì một bản *luôn nói có* sẽ xanh qua cả bốn hàng kia. Chrome sạch, không tốn credit |
| `scripts/chrome-do.mjs` | **Dụng cụ chung của hai phép đo trên**: mở một Chrome hồ sơ TRỐNG trong thư mục tạm, nói chuyện qua ống điều khiển, giết sạch khi xong. Nó gọi những method CDP mà extension **không bao giờ có** (`Page.crash`, `Target.createTarget`) — hợp lệ vì Chrome này do chính nó đẻ ra, không phải Chrome của Đức. **Đừng import từ mã chạy trong extension** (nó dùng `node:child_process`) |
| `scripts/do-tai-len.mjs` | **PHÉP ĐO ⑤** (`T29`/`W8`) — `input.upload` có thật sự gắn được file vào ô chọn tệp không. Đọc lại bằng một đường KHÁC đường ghi. Nó tìm ra một chuyện không có trong tài liệu CDP: **Chrome nhận cả đường dẫn KHÔNG TỒN TẠI** và gắn một tệp rỗng 0 byte mà không báo lỗi — đó là lý do máy chủ Udin phải `statSync` |
| `scripts/do-hinh-hoc.mjs` | **PHÉP ĐO ④** (`T10`/`S-21`) — khi nào một tab THÔI trả lời câu hỏi hình học. Giết tiến trình vẽ trang rồi hỏi lại; tái hiện đúng hình dạng 12/09 và chứng minh **một lượt điều hướng chữa được**. Cách nhận ra đã vào `README.md` |
...
| `tests/kiem-nhanh-smoke.mjs` | năm bước · từng câu *“làm gì tiếp”* · `engine` giả **áp đúng chữ ký** |
| `scripts/kiem-cai-dat.mjs` | **BẢN CÀI CỦA BẠN có chạy không** — sáu bước, dừng ở bước đầu tiên hỏng, mỗi bước hỏng nói luôn phải làm gì. Gõ cửa đúng Bridge đang chạy và đúng extension đã nạp; **khác hẳn** `scouter-bridge-live-check.mjs`, file đó dựng máy chủ riêng để đo MÃ. Hai chốt đừng đảo: công tắc ghi **ĐÓNG không phải hỏng** (mặc định, luật gói số 8), và bước ① hỏi `bridge.sessions` chứ **không** `system.ping` — ping đi tới extension nên nó chết vì đúng cái nó đi tìm (`G-91`) |
...
| `../pilots/vizcom-anhducds/` | **Pilot E2E 18/09**: Local AI → Bridge → đúng ghế → Scouter → đúng target Vizcom của `anhducds` → adapter → đọc → verify. Chứng minh chọn được **1 trong 3 tài khoản cùng URL** và LOẠI hai cái kia, kèm bộ đếm lệnh ghi = 0. Bằng chứng: `ket-qua-2026-09-18.txt`. Trong đó `chay.mjs` = chọn-đúng-một, `bam-generate.mjs` = **pilot BẤM Generate** (neo theo `id` React của từng khối ⇒ khớp đúng một; xác nhận khối bằng chữ trong ô prompt, không bằng thứ tự; ảnh chụp ra ĐĨA nên 0 byte vào context; khai thẳng chỗ KHÔNG chứng minh được danh tính) · `do-bcb-workbench.mjs` = **study graph Workbench** (bề mặt CHÍNH của Đức; graph nằm trong `<canvas>`, không trong DOM — `scout.shot` là giác quan duy nhất cho ảnh gốc/ảnh ra/cạnh nối; báo cáo ở `v0.1.0/docs/WORKBENCH-GRAPH.md`) · `do-bcb.mjs` = **benchmark chi phí context** (3 ca × 3 cách nhìn; chạy đúng trên bề mặt nào đang mở; báo cáo ở `v0.1.0/docs/CHI-PHI-CONTEXT.md`) · `ghi.mjs` = **pilot GHI** (3 lệnh ghi trên đúng `anhducds`, hai tài khoản kia 0; khoá danh tính 4 lượt; bằng chứng `ket-qua-ghi-2026-09-18.txt`) · `kiem-ke-tai-khoan.mjs` = **kiểm kê CẢ BA tài khoản** cho phép thử cách ly (read-only, bộ đếm lệnh ghi theo từng target, lọc email theo HÌNH DẠNG chứ không gõ cứng địa chỉ) |
