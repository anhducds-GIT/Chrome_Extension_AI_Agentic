...
| I11 | **Kéo-thả một TỆP vào một phần tử** | `input.tha` (lõi ghi; lên dây **chỉ ở gói Udin**, y như `I9`) | **ĐÃ CHỨNG MINH** | Mở 16/09 khuya vì `I9` bật hộp thoại `Open` lên màn hình Đức mỗi lượt — lời khai *“hộp thoại không hiện”* của `W8` **SAI**, Đức gửi ảnh chụp hai lần trong một tối, và **ba giả thuyết sửa đều trượt** (tắt chặn sớm · trang bấm trễ · thiếu `Page.enable`). Đường này **không đi qua hộp thoại nào cả** — khác biệt về CẤU TẠO, không phải một lượt chặn khéo hơn. Đo trên Chrome hồ sơ trống: trang nhận đủ `dragenter → dragover → drop`, `files.length = 1`, tên đúng, **525119 byte khai VÀ đọc thật ra cũng 525119**, kiểu `image/jpeg`, **0 hộp thoại** trước lẫn sau. **Không mở thêm quyền đọc đĩa:** tệp đi qua đúng cổng của `I9` — `path_tuyet_doi` do MÁY CHỦ đặt, nhốt trong vùng ghi. **Toạ độ vẫn do ta tính:** method CDP nhận `x`/`y`, và cổng chung của lõi ghi **TỪ CHỐI THẲNG** (`COORDINATE_NOT_ACCEPTED`) mọi toạ độ người gọi tự điền — đo lại ở đây vì đây là method đầu tiên của gói thật sự đưa `x`/`y` xuống CDP | |
| I10 | **Chọn thêm một phần tử** (Shift+click) | `scout.chon` | **ĐÃ CHỨNG MINH** | Mở 16/09 vì một nhu cầu đo được: trên Udin, Shift+click là cách dựng **THỨ TỰ tham chiếu**, và prompt gọi chúng bằng `@1` / `@2` — câu thật Đức chụp màn hình là *“APPLY STYLE OF @1 TO @2”*. **Không thêm method CDP nào**: vẫn `Input.dispatchMouseEvent`, chỉ thêm mặt nạ `SHIFT` **gõ cứng trong lõi ghi** — cùng khuôn `Ctrl+A` của `I4`, và cảnh báo *“đừng mở tham số `modifiers`”* vẫn đứng nguyên: nó chặn một THAM SỐ TỰ DO, không chặn một THAO TÁC CÓ TÊN. Khai `da_kiem: false` và nói rõ nó không biết phần tử mang số mấy. **Giá phải nói rõ:** Shift+click lên thẻ liên kết mở cửa sổ mới, nên nó chỉ bắn sau cổng selector và phép kiểm điểm bấm | |
...
**ĐÃ CHỨNG MINH 28 · MỘT PHẦN 3 · CÓ 6 · CHƯA CÓ / CHƯA ĐO 7 · ĐÃ BỎ 1.** Tổng 45 dòng.
**Seed Coverage = 28 / 44** (không tính dòng ĐÃ BỎ; MỘT PHẦN không tính là đạt).
...
|---|---|---|---|---|
| W1 | Vượt màn "User Limit Reached" | O4 O6 I1 | **ĐẠT** 13/09 · **bấm lại 17/09** | `qua-man-cho.mjs` · trước: màn chắn có · thao tác: chờ nút `usable` → bấm → kiểm màn chắn đã tắt → **bấm lại, tới 12 lượt** · thành công: màn chắn hết + ô prompt `usable`, trả về số lượt đã bấm · thất bại: hết 12 lượt mà màn chắn còn — và câu báo **KHÔNG đặt tên cho nguyên nhân**. ~~*"máy chủ Udin đầy chỗ"*~~ là một nguyên nhân lệnh này không kiểm được, và **Đức bác 17/09**: *"đây chỉ là bug thôi, từ sau bạn cứ ấn."* · `G-28` |
...
| W8 | Tải ảnh tham chiếu lên | I9 | **ĐẠT** 16/09, chạy thật | ~~**CHẶN**~~ — chữ *“chặn”* sai: không có gì chặn nó, `T29` **làm được ngay** và phần khó nhất (`trongGoc` kiểm đường dẫn ở máy chủ) đã xây xong và đã ghim. Nó **chưa được mở**, và đó là một quyết định — `D4` chốt 16/09. Đếm bằng đúng thước đã đóng `T6`: **0 việc đang chạy cần đưa tệp vào trang**, và `W8` vốn xếp NGOÀI danh sách `W` bắt buộc từ 14/09. Cửa này là cửa **đầu tiên đi từ đĩa ra một trang web**, nên không mở vì *“sắp tới chắc sẽ cần”*. ~~**Mốc mở:** ngày Đức cần đưa một ảnh tham chiếu vào Udin~~ → **MỐC ĐÃ CHẠM cùng ngày**: Đức chốt *“lấy ảnh Udin đã tạo, đưa vào, xin style khác”*. `T29` làm xong, `scout.upload` có thật và **đã chạy thật trên Chrome sạch** (file đúng tên, đúng 54 byte, vào đúng ô selector chỉ). **NHƯNG `W8` vẫn CHẶN, bởi một thứ khác hẳn:** đo trang Udin 16/09 thì **không có một `<input type=file>` nào** — cả lúc menu *Add to canvas* đóng lẫn lúc mở (`[class*=drop]` 1 → 2, ô chọn tệp 0 → 0). Năng lực đúng; trang không có cái ô để đổ vào. Giả thuyết CHƯA ĐO: Udin dựng ô tạm trong chính lượt bấm, hoặc dùng cửa chọn tệp của trình duyệt. **Đường đo AN TOÀN** là `Page.setInterceptFileChooserDialog` → **Đức chốt CHO MỞ 16/09**, sau khi phép đo trả lời ba câu cùng lúc: nó chạy **không cần `Page.enable`**, **không cần kênh sự kiện**, và khi hộp thoại bị chặn thì **ô nhận file nằm lại trong DOM chờ** — nên hỏi lại là thấy. Một method, không kéo theo cái nào. `scout.upload` nay có **hai đường**, chọn đúng một: `selector` (ô có sẵn) · `mo_bang` (nút phải bấm để trang dựng ô ra — ca Udin). Đường `mo_bang`: **chặn trước, bấm sau, tắt trong `finally`** — hiểm thật của method này không phải quyền đọc file mà là **để quên nó bật**, lúc ấy hộp thoại chính Đức mở cũng im lặng không hiện. 14 khối ghim · 12 đột biến · 181/181.
**CHẠY THẬT 16/09, không một cú bấm nào của người:** đưa ngược tấm `02-batch-…jpg` mà chính Udin sinh ra hôm ấy vào lại Udin. `mo_bang` trỏ mục `Image` **tìm theo NHÃN** (không theo số thứ tự) · ~~hộp thoại hệ điều hành **không hiện lên màn hình Đức**~~ — **LỜI KHAI NÀY SAI, gạch 16/09 tối.** Nó chưa bao giờ được đo: tôi suy ra từ việc Đức không kêu. Đức báo và **gửi ảnh chụp**: hộp thoại `Open` của Windows CÓ bật lên, đúng thư mục ảnh của lượt chạy, và Đức phải bấm **Cancel**. Lượt tải lên vẫn thành công (ô chọn tệp sống trong lúc hộp thoại treo), nên nó hỏng theo kiểu **im lặng** — *không một cú bấm nào của người* là sai, giá đúng là **một cú Cancel mỗi lượt** · pill đính kèm `0 → 2`, nhãn đọc ra `"1 Image"` · ô chọn tệp tạm bị trang thu lại (`0`), đúng như khi người dùng tự chọn · `hit: descendant`, tiêu 5 đơn vị trần ghi.
~~**Hợp đồng:** … đếm ô chọn tệp → **chặn hộp thoại** → bấm nút mở → tìm ô **MỚI** → đổ file → **tắt chặn trong `finally`** … `NO_FILE_CHOOSER` · `UPLOAD_MODE_UNCLEAR`~~ — **hợp đồng ấy ĐÃ GỠ 17/09.**
**ĐƯỜNG `mo_bang` VÀ `Page.setInterceptFileChooserDialog` GỠ HẲN 17/09.** Không phải vì chúng hỏng — chúng chạy đúng như khai. Vì `scout.tha` (kéo-thả, mở 16/09 khuya) làm cùng việc mà **không đi qua hộp thoại nào cả**, nên đường cũ chỉ còn là một cách đắt hơn để tới cùng chỗ, và nó mang theo cái hiểm thật của method chặn: **để quên nó bật thì hộp thoại chính Đức mở cũng im lặng không hiện.** Một method GHI không ai gọi vẫn là một method GHI còn mở. `WRITE_CDP_METHODS` **17 → 16**. Mở lại = đổi luật an toàn = hỏi Đức.
**Hợp đồng NAY:** trước: tab qua màn chờ, ảnh nằm trong vùng ghi, **trang đã có sẵn một `<input type=file>`** · thao tác: selector khớp đúng một → hỏi Chrome xác nhận nó là ô chọn tệp thật → đổ file · thành công: đúng một ô nhận đúng một file · thất bại: `SELECTOR_REQUIRED` (không khai `selector`) · `SELECTOR_AMBIGUOUS` · `NOT_A_FILE_INPUT` · `PATH_OUTSIDE_ROOT`/`FILE_NOT_FOUND` (chặn ở **máy chủ**). Trang KHÔNG có sẵn ô nào — ca của Udin — thì dùng `scout.tha`, đó nay là đường chính. 8 khối ghim · 10 đột biến · 179/179 |
...
**8 / 8 workflow ĐẠT · 0 CHẶN · E2E ĐẠT.** Cả bảng xanh, 16/09.

**Đây là chỗ Udin chuyển sang `MASTERED`**, và điều kiện ấy do chính file này viết ra từ 13/09:
*"mọi workflow bắt buộc ĐẠT, không còn CHẶN, và E2E ĐẠT"*. Ba vế đều đạt. Danh sách bắt buộc Đức
chưa ký thành văn, nhưng danh sách **đề xuất** (`W1 W2 W3 W4 W7` + E2E) đã ĐẠT từ 14/09, và ba
hàng từng để NGOÀI (`W5` `W6` `W8`) nay cũng ĐẠT — nên không còn cách đọc nào làm nó chưa xong.

**Và ba hàng ấy hết chặn vì ba lý do KHÁC NHAU, không cái nào là lý do đã ghi:** `W6` bị chặn bởi
một **thao tác không tồn tại**; `W5` bởi một **cột "Cần" khai sai**; `W8` bởi **chỗ đặt sai** rồi
tới **một ô nhận file chỉ sống trong lúc hộp thoại mở**. Cả ba dòng cũ đều đổ cho `S-22`. Xem
`TRIALS.md` 16/09.
