...
**Hợp đồng:** trước: tab qua màn chờ, ảnh nằm trong vùng ghi · thao tác: đếm ô chọn tệp → **chặn hộp thoại** → bấm nút mở → tìm ô **MỚI** → đổ file → **tắt chặn trong `finally`** · thành công: đúng một ô mới nhận đúng một file · thất bại: `NO_FILE_CHOOSER` (bấm mà trang không dựng ô nào) · `SELECTOR_AMBIGUOUS` (dựng ra nhiều ô — không đổ vào *"cái đầu tiên"*) · `UPLOAD_MODE_UNCLEAR` · `PATH_OUTSIDE_ROOT`/`FILE_NOT_FOUND` (chặn ở **máy chủ**) |
