/* Chạy mọi phép ghim của gói `udin-optic` trong một lượt.
 ...
 *   ② `../tu-dong/tests/*.mjs`  — phép ghim của tầng chạy việc, nằm NGOÀI thư mục phiên bản
 *
 * Tầng chạy việc đứng ngoài thư mục phiên bản là cố ý: nó là script Node gọi Bridge từ dòng
 * lệnh, không phải mã chạy trong trình duyệt, và nó không lên phiên bản cùng nhịp với extension.
 */
...
const tangChayViec = path.resolve(here, "..", "..", "tu-dong", "tests");
/* Thư mục biến mất thì ĐỎ, không lặng lẽ bỏ qua: một lượt chạy 0 tệp của tầng chạy việc đọc y
 * hệt một lượt chạy tốt, và đó đúng là cách một suite tự tắt mà bảng vẫn xanh. */
if (!fs.existsSync(tangChayViec)) {
  console.log(`FAIL  không thấy thư mục phép ghim của tầng chạy việc: ${tangChayViec}`);
  process.exit(1);
}
for (const ten of fs.readdirSync(tangChayViec).sort()) {
  if (ten.endsWith(".mjs") && !boQua.has(ten)) files.push(path.join(tangChayViec, ten));
}
