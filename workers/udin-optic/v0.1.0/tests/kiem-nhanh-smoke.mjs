...
import { kiemNhanh, SO_METHOD } from "../scripts/kiem-nhanh.mjs";
...
const MIEN = "vinfast.udinbv.com";
const tabUdin = { targetId: "T1", url: `https://${MIEN}/canvas`, type: "page" };
...
const chay = (ghiDe) => {
  ...
  return kiemNhanh(do_, { mien: MIEN }).then((k) => ({ ...k, daGoi }));
};
...
/* ═══ ⓐ ĐƯỜNG TỐT ══════════════════════════════════════════════════════════ */
{
  ...
  assert.match(k.buoc[2].noi, new RegExp(`1 tab ${MIEN}`), "phải nói rõ có mấy tab Udin");
 ...
 * chạy", KHÔNG nói "lỗi", và phải khác hẳn câu "chưa chọn tệp ghép cặp". */
{
  ...
  assert.match(tat.buoc[0].lamGi, /Chay-may-chu-Udin\.cmd/, "phải chỉ ra ĐÚNG tệp cần chạy");
  ...
  assert.match(chuaGhep.buoc[0].lamGi, /tao-tep-ghep-cap\.mjs/);
...
/* ═══ ⓐ BƯỚC ③ — có tab web nhưng KHÔNG có tab Udin thì vẫn đi tiếp, có báo ═ */
{
  ...
  assert.match(k.buoc[2].noi, new RegExp(`KHÔNG có tab ${MIEN}`), "nhưng phải NÓI RA là chưa mở trang Udin");
}
...
/* ═══ ⓐ BƯỚC ④ — ưu tiên tab Udin, và hỏng thì nói đúng nguyên nhân ════════ */
{
  ...
  assert.deepEqual(daDo, [tabUdin.targetId],
...
assert.equal(capabilities().methods.length, SO_METHOD,
  "`SO_METHOD` phải khớp bảng lệnh thật — hai con số gõ ở hai chỗ thì sớm muộn lệch nhau");
