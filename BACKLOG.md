...
- **ĐÓNG Y-12 + Y-07** · 2026-09-07 · lane `claude-cong-nhanh` · đệm lệnh git theo bộ đọc trong `createHeadDeps` (`scripts/build-dashboard.mjs`, commit `36c7423`). **Đo lại trước khi sửa và số trong sổ SAI:** sổ ghi 392 giây, máy này ra **640**. Trước → sau, cùng máy, lượt sạch: `tests/build-overview-smoke.mjs` **640 → 17 giây**, `tests/build-dashboard-smoke.mjs` **78 → 77**, một lượt `buildOverview` **11,5 giây** lượt đầu và **0,087 giây** lượt sau trên cùng bộ đọc. **Không làm yếu gì:** 33/99 phép kiểm và 393/456 khẳng định **y nguyên** — không file nào dưới `tests/` bị chạm. **Đột biến kiểm 8 con** (cả 8 mỏ neo khớp đúng 1 chỗ, cả hai bản) chạy trên bản cũ tại `fc66d38` (control 33/33 + 99/99 xanh) và bản mới: **tập bị bắt giống hệt** — bắt `M1 M4 M5 M7`, sót `M2 M3 M6 M8`. Ba artifact máy sinh vẫn khớp HEAD, và hai bộ đọc mới dựng cho ra trang giống hệt từng byte.

## N-27 · Bốn đột biến vào bộ đọc HEAD lọt lưới cả hai suite gốc

- **mở:** 2026-09-07 · lane `claude-cong-nhanh`
- **vùng:** `_code`
- **đóng khi:** lệnh: bốn đột biến dưới đây làm `npm test` ĐỎ — hoặc Đức chốt rằng ba trong số
  đó không đáng ghim và chỉ giữ lại con đáng nhất.
- **đo được 2026-09-07, cả TRƯỚC và SAU lượt tăng tốc, y nhau** (nên đây là lỗ có từ trước, không
  phải hậu quả của bộ đệm): bốn đột biến vào `scripts/build-dashboard.mjs` và
  `scripts/build-overview.mjs` chạy hết cả suite mà **không phép kiểm nào đỏ**:
  ⑴ `isFile` luôn trả `true` · ⑵ bộ lọc của `listDirs` bị đảo · ⑶ `headDate` trả một ngày cứng
  `"2020-01-01"` · ⑷ `debtTotal` bị ghim về `0`.
- **con nào đáng nhất:** ⑶ và ⑷. `headDate` là mốc mọi phép tính tuổi trên bảng dựa vào, và
  `debtTotal` là con số nợ Đức đọc — cả hai sai thì bảng nói dối mà cổng vẫn xanh.
- **vì sao ⑴⑵ khó hơn:** hai suite chạy trên repo THẬT, nên mọi đường dẫn hỏi đều tồn tại và
