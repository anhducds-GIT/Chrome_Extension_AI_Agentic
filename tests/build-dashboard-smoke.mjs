...
/* Gốc repo THẬT — nơi đọc mã nguồn ra để biết repo tạm cần chép những file nào. */
...
/* 23. Gate 7 integration: HEAD-only, read-only, không bị --quick bỏ, và bắt artifact stale. */
{
  ...
  try {
    ...
    for (const name of fileScriptCanChep(TEST_ROOT, ["repo-structure.mjs", "handoff.mjs", "build-dashboard.mjs", "feature-parity.mjs", "session-check.mjs", "claim.mjs", "backlog-check.mjs", "chay-test.mjs"])) {
    ...
    /* ĐẢO LẠI 18/09 (N-65) — và đây là "freshness/artifact semantics HIỆN TẠI" Đức chốt giữ.
     *
     * Bản cũ ghim quyết định K2-8 (03/09): *"artifact cũ KHÔNG còn chặn cổng lane, chỗ chặn dời
     * sang safe-push"*. `4da1e9e5` ĐẢO quyết định đó — bài này nằm trong khu cách ly nên không
     * ai thấy. ĐO 18/09 trên chính fixture này: hàng cổng ra `[ĐỎ  ] Sự thật máy sinh còn tươi`.
     *
     * Không "khôi phục" K2-8 bằng cách nới phép ghim: hợp đồng của cổng là thứ Đức chốt, không
     * phải thứ một phép ghim cũ giữ hộ. Ghim THỨ ĐANG CHẠY, và ghim luôn cả chất lượng câu báo —
     * một hàng đỏ không nói được sửa bằng lệnh nào là một hàng đỏ sẽ bị tắt. */
    assert.match(stale.stdout, /\[ĐỎ  \] Sự thật máy sinh còn tươi/,
      "artifact da commit bi cu thi cong lane ĐỎ — day la hop dong hien tai, doi no phai hoi Duc");
    assert.match(stale.stdout, /build-dashboard\.mjs không khớp với HEAD/, "phai NOI TEN bo sinh nao lech");
    assert.match(stale.stdout, /feature-parity\.mjs không khớp với HEAD/, "va ke DU, khong dung o cai dau tien");
    assert.match(stale.stdout, /DASHBOARD\.md lệch tại dòng \d+/, "phai chi ra DONG nao lech, khong chi noi 'co lech'");
    assert.match(stale.stdout, /node scripts\/build-dashboard\.mjs && node scripts\/feature-parity\.mjs/,
      "va phai dua ra LENH SUA dung, dung danh sach bo sinh da khai");
    ok("Gate 7 dùng HEAD, chỉ đọc, không bị --quick bỏ; artifact đã commit bị cũ thì cổng lane ĐỎ, nêu tên · dòng · lệnh sửa");
  } finally {
