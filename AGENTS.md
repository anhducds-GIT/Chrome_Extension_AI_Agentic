...
| **Sửa một file mà BỘ KHUNG cũng có bản của nó** (`claim.mjs` · `handoff.mjs` · `session-check.mjs`…) | Cửa riêng của repo đặt trong đó **bị lượt migrate sau xoá**, và phép ghim đặt cùng chỗ chết theo — hai cửa đã mất kiểu đó (`N-68`). Ghim cửa riêng ở `tests/cua-rieng-repo-smoke.mjs`; `tests/import-gay-smoke.mjs` canh luật *không ai được import một cái tên đã chết* — **không có danh sách miễn trừ**, khu cách ly đã rỗng từ `N-65` (18/09) và `tests/dau-suite-smoke.mjs` đỏ nếu một khu sống lại dưới bất kỳ tên nào có tiền tố `test:chet|hong|cach-ly`. **Trợ thủ chỉ-dùng-cho-test KHÔNG đặt trong `scripts/`** — `tests/_chep-script.mjs` (danh sách script phải chép vào repo tạm) từng là `fileScriptCanChep` của `repo-structure.mjs` và chết theo lượt migrate, kéo **469 vế** của `build-dashboard-smoke` vào khu cách ly. Tệp `tests/_*.mjs` là trợ thủ, không phải một bài, nên `dau-suite-smoke` miễn cho nó khỏi luật *mọi bài phải nằm trong một chuỗi* — nay có hai: `tests/_chep-script.mjs` và `tests/_dong-ho-lui.mjs` (đẩy đồng hồ của một tiến trình, để `tests/artifact-tat-dinh-smoke.mjs` chứng minh bản sinh đã commit **không nhìn đồng hồ**, `N-70`) |
<!-- BAN-DO:END -->
