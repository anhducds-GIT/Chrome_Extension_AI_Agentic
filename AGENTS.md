...
| **Cho người khác xem BỘ KHUNG là gì (không phải repo này đang tới đâu)** | `node scripts/build-template-overview.mjs <file-ra.html>` — trang mô tả chính bộ khung ở trạng thái sạch. Khác hẳn `DASHBOARD.md`: cái kia nói *dự án chạy tới đâu*, cái này nói *bộ khung gồm gì, vận hành ra sao*, và nó **rỗng có chủ đích**. Mọi con số sinh từ `buildTemplateFiles()` nên trang không thể nói khác bộ khung. **Bản ra KHÔNG commit** — nó để publish và tự in ngày sinh |
| **Quy trình: kiểm một repo, rồi đưa nó lên chuẩn** | `docs/protocols/KIEM-MOT-REPO.md` (kiểm thì rẻ, chuyển thì đắt — bao giờ cũng kiểm trước) · `docs/protocols/CHUYEN-REPO-LEN-CHUAN.md` (sáu bước, bốn cạm bẫy đã xảy ra thật, nghiệm thu bằng máy). **Quy trình thứ hai CHƯA từng chạy trên repo thật khác nghề** — vài bước sẽ sai, và lần chạy đầu là để tìm ra chúng. Vì sao hai công cụ này ở lại repo nhà chứ không đi theo bản trích: `docs/adr/0002-cong-cu-va-quy-trinh-o-repo-nha.md` |
